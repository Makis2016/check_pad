...
class InnerDataTable extends BaseComponent {
    ...
    _renderRow(rowData, rowId) {
        const { header, rowSelection, onRowCheckedChange, onSelectRow, isExpanded,rowHeight } = this.props;
        const selection = rowSelection && rowSelection.type === 'checkbox' ? <CheckBox key="choose" vo={this.data[rowId]} vmodel="_checked" onChecked={onRowCheckedChange} /> : null;

        return (
            <Row style={styles.row} expandedRowStyle={{borderBottomWidth:0,borderColor: 'gray',height:rowHeight}} rowId={rowId} vo={this.data[rowId]} vmodel="_selected" isExpanded={isExpanded} renderExpansion={() => this._renderExpansion(rowData,rowId)}>
                {selection}
                {header
                    .filter((item) => item.visible)
                    .map((item, index) => (
...
export default class DataTableEx extends BaseComponent {
    ...
    _render() {
        ...
        const { rowSelection, header, isExpanded, renderExpansion, fillParent,expandedRowHeight } = this.props;
            ...
            <View style={[styles.container, contentContainerStyle]}>
                ...
                <ScrollView style={styles.container} contentContainerStyle={contentContainerStyle} horizontal={true} showsHorizontalScrollIndicator={true}>
                    <View style={{ flex: 1, minWidth: 1 }}>
                        ...
                        <InnerDataTable rowSelection={rowSelection} header={header} width={this.state.tableWidth} rowHeight={isExpanded ? expandedRowHeight : 35} data={this.data} dataSource={this.state.dataSource} isExpanded={isExpanded} renderExpansion={renderExpansion} onRowCheckedChange={() => this._onRowCheckedChange()} onSelectRow={(rowId) => this._onSelectRow(rowId)} />
                    </View>
                </ScrollView>
            </View>
        );
